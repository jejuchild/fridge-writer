...
export default function KitchenPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f8fafc] text-slate-800">
      <div className="kitchen-bg flex-1 flex flex-col max-w-md mx-auto w-full relative shadow-2xl overflow-y-auto pb-28">
        <div className="pointer-events-none absolute inset-0 sun-glare" />

        <header className="sticky top-0 z-30 bg-white/75 backdrop-blur-xl border-b border-white/70 p-4 flex items-center justify-between">
        ...
        <main className="relative z-10 p-4 space-y-6">
          <section className="retro-fridge rounded-[2.2rem] overflow-hidden border-4 border-white shadow-xl relative">
              ...
              {COOK_MODES.map(({ mode, icon, label }) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-2xl py-3 px-1 border transition-all flex flex-col items-center gap-1.5 ${
                      isActive
              ...
              {WRITING_STYLES.map(({ style, icon, label, desc }) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-2xl py-2.5 px-3 border transition-all text-left ${
                      isActive
                  ...
                  >
                    ...
                    <p className="text-[10px] mt-1">{desc}</p>
                  </button>
                );
              })}
            </div>
          ...
          <section className="space-y-3">
            <div className="flex items-center justify-between px-1">
              <h2 className="text-lg font-extrabold tracking-tight text-slate-900">믹싱 볼</h2>
              <span className="material-symbols-outlined text-amber-500">restaurant</span>
            </div>

            <div className="wood-counter rounded-3xl p-1 shadow-lg">
              <div className="bg-white/95 rounded-2xl border border-white shadow-inner p-5 min-h-[220px] flex flex-col">
                <textarea
                  value={prompt}
                  onChange={(e) => setPrompt(e.target.value)}
                  className="flex-1 bg-transparent border-none focus:ring-0 focus:outline-none text-slate-800 text-lg font-medium placeholder:text-slate-300 resize-none p-0"
          ...
          <div className="flex flex-col items-center justify-center py-5">
            ...
            >
              <div
                className={`w-24 h-24 bg-amber-300 rounded-full shadow-xl shadow-amber-300/30 flex items-center justify-center text-white border-4 border-white transition-transform active:scale-95 group-hover:scale-105 ${
                  isLoading ? "animate-pulse" : ""
                }`}
              >
                <span
                  className={`material-symbols-outlined text-5xl font-light ${
              ...
              <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 bg-white px-5 py-2 rounded-full shadow-lg border border-slate-50 whitespace-nowrap">
